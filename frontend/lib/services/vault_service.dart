...
class VaultService {
  ...
  Future<VaultItem> createVaultItem(CreateVaultItemRequest req) async {
    try {
      ...
      _logger.e('Create vault item failed: ${e.response?.data}');
  ...
  Future<List<VaultItem>> listVaultItems(String groupId,
    ...
    try {
      final r = await _dio.get('/vault', queryParameters: {
        'group_id': groupId,
        'limit': limit,
        'offset': offset
      });
      ...
      _logger.e('List vault items failed: ${e.response?.data}');
      throw _handleError(e);
    }
  }
  ...
  Future<VaultItem> getVaultItem(String id) async {
    try {
      ...
      _logger.e('Get vault item failed: ${e.response?.data}');
      throw _handleError(e);
    }
  }
  ...
  Future<void> deleteVaultItem(String id) async {
    try {
      ...
      _logger.e('Delete vault item failed: ${e.response?.data}');
      throw _handleError(e);
    }
  }
