...
func (r *FinanceRepo) CreateExpense(ctx context.Context, e *models.Expense) error {
	...
	`, e.ID, e.GroupID, e.PayerID, e.Amount, e.Description, e.Category, e.Currency, e.Date, e.CreatedAt, e.UpdatedAt)
	return err
}
...
func (r *FinanceRepo) GetExpenseByID(ctx context.Context, id uuid.UUID) (*models.Expense, error) {
	row := r.pool.QueryRow(ctx, `
		SELECT id, group_id, payer_id, amount, description, category, currency, date, created_at, updated_at
		FROM expenses
		WHERE id = $1
	`, id)

	var e models.Expense
	err := row.Scan(&e.ID, &e.GroupID, &e.PayerID, &e.Amount, &e.Description, &e.Category, &e.Currency, &e.Date, &e.CreatedAt, &e.UpdatedAt)
	if err != nil {
...
func (r *FinanceRepo) UpdateExpense(ctx context.Context, e *models.Expense) error {
	...
	cmd, err := r.pool.Exec(ctx, `
		UPDATE expenses
		SET payer_id = $1, amount = $2, description = $3, category = $4, currency = $5, date = $6, updated_at = $7
		WHERE id = $8
	`, e.PayerID, e.Amount, e.Description, e.Category, e.Currency, e.Date, e.UpdatedAt, e.ID)
	if err != nil {
		return err
	}
